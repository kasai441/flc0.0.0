...
class HomePageController < ApplicationController
  def index
    if logged_in?
      ...
      unless (total_time = @user.total_time).nil?
        @total_time = (total_time / 60).to_i
        if (practice_days = @user.practice_days).positive?
          ...
          if waitday.positive? && @total_time.positive?
            @adding_guide = ((@answer_time * quantity / waitday) / @total_time).to_i
            @adding_guide = 1 if @adding_guide < 1
          end
        end
      end
    else
      ...
      if cookies[:quizcards_today_ids]
        ...
        cookies[:quizcards_today_ids] = JSON.generate(@quizcards_today.map(&:id)) if @quizcards_today.present?
      end
      @quizcard = @quizcards_today.first if @quizcards_today
